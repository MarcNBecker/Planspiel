...
/**
 * Klasse zur Simulation der Unternehmenskette. Diese ist die oberste Einheit
 * eines Spielers und ist direkt dem Spiel untergeordnet.
 * 
 * @author Marc Becker
 * 
 */
public class Unternehmenskette {
	...
	/**
	 * Erzeugt ein Unternehmen mit den Spiel Startwerten für Kapital, Gehalt und
	 * Entlassungskosten
	 * 
	 * @param name
	 *            Name des Unternehmens, dieser wird vom Spieler zu Beginn des
	 *            Spiels festgelegt
	 */
	 ...
	 * 
	 * @param standort
	 *            Standort an dem das Filialeobjekt erzeugt werden soll
	 */
	public Filiale eroeffnenFiliale(Standort standort) {
		...
		// Der Konstrukter von Filiale ruft die entsprechende Standort-Methode
		// auf
	...
	/**
	 * Entfernt eine Filiale ohne Mitarbeiter vom Standort und der
	 * Unternehmenskette
	 * 
	 * @param filiale
	 *            Filiale, die geschlossen werden soll
	 */
	...
	/**
	 * Berechnet das gesamte Kapital der Unternehmenskette. Dieses setzt sich
	 * aus der Kasse (Kapital), dem Wert der Filialen und dem Wert des Lagers
	 * zusammen
	 * 
	...
	/**
	 * Berechnet das Fremdkapital im Unternehmen. Dies entspricht den
	 * Restbeträgen aller Kredite
	 * 
	...
	/**
	 * Fügt den Kredit dem Unternehmen hinzu, wenn der Betrag noch im Rahmen der
	 * Kreditwürdigkeit liegt
	 * 
	 * @param betrag
	...
	/**
	 * Hier wird geprüft welchen Kreditbetrag der Spieler maximal aufnehmen
	 * kann. Dabei wird das Verhältnis 1:3 von FK:EK eingehalten
	 * 
	...
	public boolean verbuchenKosten(Kostenverursacher verursacher, double betrag) {
		...
		// Die Reports werden an der Stelle "Runde des Reports" - 1
		// gespeichert!!
	...
	/**
	 * Berechnet alle laufenden Kosten des Unternehmens und muss vom Spiel
	 * aufgerufen werden Diese Kosten sind Filialkosten und Kredittilgungskosten
	 */
	public void berechnenKosten() {
		...
		/*
		 * for (int i=0; i<holeKreditListe().size(); i++) { Kredit kredit =
		 * holeKreditListe().get(i); kredit.tilgen(); }
		 */
		// Setzt die Schleifenlänge immer wieder neu und berücksichtigt somit
		// Kredite, die bei der Tilgung von Krediten aufgenommen werden müssen
		int i = 0;
	...
	public void setzeKasse(double kasse) {
		...
		if (kasse >= 0) {
			...
			// Diese Formel berechnet den Betrag der notwendig ist um den
			// fehlenden Betrag und die erste Tilgung und Zins des Kredit zu
			// tilgen
			...
			boolean kredit = aufnehmenKredit(aufnahmeBetrag); // Versuche einen
																// Kredit
																// aufzunehmen
			if (!kredit) { // Versuch fehlgeschlagen => Pleite, da keine
							// Kreditwürdigkeit mehr
				setzePleite(true);
			} else {
	 ...
	 * 
	 * @param entlassungskosten
	 *            Kosten größer gleich 0
	 */
